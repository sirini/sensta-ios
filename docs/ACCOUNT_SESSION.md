...
| 내 작품 스튜디오 | `GET /board/my/studio?id=photo&page&limit&sort`, Bearer access token | JWT 본인의 공개·비공개 작품, 집계와 정렬·페이지 정보 |
...
- 내 작품 스튜디오는 사용자 UID를 query나 body로 보내지 않는다. 서버가 JWT UID로 본인 범위를 정하고
  삭제글·공지는 제외하되 공개·비공개 작품은 함께 반환한다. 비공개 작품 상세와 댓글도 같은 인증 세션으로
  조회하며, 응답 게시글 UID를 다시 확인한다. 썸네일은 설정된 API와 같은 origin의 로컬 업로드 경로만
  해석한다.
...
내 작품 스튜디오 QA: 로그인 → 계정 화면 → 내 작품 스튜디오 → 작품·사진·받은 좋아요 집계 확인 →
최신순·조회순·좋아요순·댓글순 전환 → 목록 끝에서 다음 페이지 확인 → 공개 작품과 잠금 표시된 비공개
작품 상세 진입 → 뒤로 가기 → 사진 업로드 진입과 업로드 완료 뒤 목록 갱신을 확인한다. 네트워크를 끊으면
기존 목록이 유지되고 재시도가 표시되는지도 확인한다.
