...
기준일은 2026년 9월 12일이다. Apple 요구 사항은 바뀔 수 있으므로 실제 제출 직전에 연결된 공식 문서를
다시 확인한다.
...
- 출시 방식: 심사 통과 후 자동 출시 (2026-09-11 제품 소유자 요청)
...
`잘못된 바이너리`와 기존 제출의 `해결되지 않은 문제` 상태를 다시 확인했다. 출시 방식은 아래의 최신 결정을 따른다.
기존 제출 ID는 `143cfbf7-a31d-4ccd-9d10-e5db7d2f1761`이다.

`1.0 (2)`는 2026-09-11 23:00 KST 업로드에 성공했고 Apple 처리 완료·제출 준비 완료를 확인했다.
기존 스크린샷·메타데이터·심사 정보를 유지하며 빌드 2로 교체한 뒤 23:04 KST 재제출해
`심사 대기 중`이었다. 2026-09-12 08:56 KST Apple은 Guideline 2.1 추가 정보를 요청했다.
현재 `심사를 통과하지 못함` 상태이며 Apple의 구체적 버그 지적은 없다. 제품 소유자의 실제 영상
촬영 중 계정 삭제 실패를 발견해 GOAPI `ed55c9b`에서 MySQL 오류 1093을 수정했다. 운영 반영과
실기기 삭제 재확인이 남았으며 iOS 빌드 2는 유지한다. 2–6번 설명은 Review Notes에 저장했고
회신·첨부·재제출은 아직 하지 않았다.
촬영 순서와 답변 초안은 [추가 자료 대응](APP_REVIEW_RESPONSE_2026-09-12.md)을 따른다. 빌드 ID는 `fc145e89-b43f-4ebb-a6a9-2cdc228d67ad`이며 위 제출 ID를 재사용한다.
재제출 당시에는 기존 결정에 따라 수동 출시였으나, 같은 날 제품 소유자가 자동 출시를 요청했다.
현재는 심사 승인 후 자동 출시하도록 설정한다.
