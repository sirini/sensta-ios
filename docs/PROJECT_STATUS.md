...
- 로그인한 사용자가 쓴 댓글에만 iOS 기본 더 보기 메뉴를 표시한다. 수정은 별도 시트에서 2자 이상을
  요구하며 Android와 같은 HTML 문자 이스케이프를 거쳐 기존 `PATCH /comment/modify` 계약으로 보낸다.
  성공한 뒤에만 화면을 갱신하고 실패하면 입력한 내용을 유지한다.
