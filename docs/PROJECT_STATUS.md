...
1. GOAPI `28d5728` runtime 전체를 운영 서버에 교체하고 재시작한다. schema와 환경 변수는 바뀌지 않아
   `install`을 다시 실행할 필요는 없다.
2. 비로그인 상태에서 Apple 인증을 다시 시도해 버튼 바로 아래 안내를 확인한다. 같은 이메일의 기존
   Google 계정 안내가 나오면 Google로 로그인한 뒤 내 계정의 Apple 계정 섹션에서 Apple ID를 연결한다.
3. 로그아웃 후 연결된 Apple ID로 직접 로그인하고 앱 종료·재실행 뒤 세션 복원까지 확인한다.
