...
- 실기기 피드백을 반영해 작품 미리보기를 기존 4:3 혼합 배치에서 모든 화면 크기의 1:1 정사각형으로
  통일했다. 일반 글자에서는 96pt 정사각형을 사용하고, 접근성 큰 글자에서는 가로 폭에 맞춘 정사각형을
  사용한다. 실제 사진의 가로·세로 비율과 관계없이 `scaledToFill` 뒤 정사각형 경계에서 자르고 12pt
  continuous rounding과 얇은 윤곽을 동일하게 적용해 행 오른쪽으로 넘치지 않는다.
- 시스템 `List`가 여러 행 사이에 짧고 비대칭으로 표시하던 기본 구분선을 제거했다. 각 작품을 16pt
  continuous rounding의 독립 카드로 나누고 좌우·상하 여백을 통일해 행 경계를 선 없이 구분한다.
- 전체 단위 테스트 111개와 일반·접근성 큰 글자 스튜디오 UI 테스트 2개, Release simulator build와
  Debug 정적 분석을 통과했다. 세 작품 목록과 큰 글자 캡처에서 정사각형·클리핑·카드 간격을 확인했고
  자동 서명 Debug 앱을 iPhone 17에 다시 설치했다.
