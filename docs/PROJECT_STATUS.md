...
1. 실제 iPhone에서 사용하지 않은 이메일로 회원가입하고 받은 6자리 인증 코드로 완료한 뒤 로그인한다.
2. 이메일 가입 QA가 끝나면 비밀번호 재설정 흐름을 다음 독립 작업으로 구현한다.
3. 이후 사진 게시·알림·계정 관리 중 Android 기능 우선순위에 맞춰 다음 수직 기능을 선택한다.
