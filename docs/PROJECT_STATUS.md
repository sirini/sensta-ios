...
- 이메일 로그인·Keychain 세션 복원·토큰 갱신·로그아웃, 게시글·댓글 좋아요와 댓글 작성·답글·본인
  ...
  확인할 수 있도록 오류 안내와 서버 진단을 보강했다. 이메일 인증 회원가입과 계정 존재 여부를 숨기는
  비밀번호 재설정도 기존 GOAPI 계약으로 연결했다. 첫 피드에는 플랫폼 공통 Sensta 앱 업적, 인앱 알림과
  GPS를 제거하는 다중 사진 업로드를 연결했다. 로그인 사용자는 집계·정렬·페이지 추가 로드와 비공개
  작품 상세를 포함한 내 작품 스튜디오를 사용할 수 있다.
...
- 공개 피드는 로그아웃 상태에서 직접 GOAPI를 호출한다. 로그인 상태에서는 Android와 같이 Bearer token을
  보내 서버의 사용자별 차단 목록을 적용하되, 사진 열람 자체의 공개 권한 계약은 유지한다.
...
2. 미확인 영구 업적의 iOS 축하 화면과 확인 처리를 연결한다.
3. 1:1 대화와 iOS push 등록, APNs/FCM 알림·딥 링크를 구현한다.
4. 신고·차단·계정 삭제를 완성하고 App Store UGC 심사 시나리오를 검증한다.
...
- 비로그인 계정 화면에 이메일 비밀번호 재설정을 추가했다. 로그인 칸의 이메일을 이어받고 기존
  `POST /auth/reset-password` JSON 계약을 인증·쿠키 없이 호출한다. 서버·Android·웹 계약 변경과 운영
  배포는 필요 없다.
- 서버가 등록되지 않은 이메일에도 같은 성공을 반환하는 정책에 맞춰 앱도 계정 존재 여부를 단정하지 않는
  완료 안내만 표시한다. 메일 링크는 기존 웹 화면에서 10분 동안 한 번만 사용할 수 있음을 안내하며,
  메일 미설정과 일반 통신 오류는 내부 정보를 노출하지 않는 문구로 구분한다.
- 전체 단위 테스트 106개와 비밀번호 재설정·이메일 로그인/가입·Google·Apple·다크/큰 글자 계정 UI
  테스트 8개가 통과했다. Release simulator build와 Debug 정적 분석, 완료 화면 캡처를 확인했고 자동
  서명 Debug 앱을 연결된 iPhone 17에 설치했다. 다음 수직 기능은 내 작품 스튜디오다.

## 내 작품 스튜디오 (2026-09-06)

- 로그인 계정 화면에서 내 작품 스튜디오로 이동한다. 기존 Android와 GOAPI의 인증된
  `GET /board/my/studio?id=photo&page=...&limit=20&sort=...` 계약을 사용하며 호출자가 사용자 UID를
  보내지 않고 서버가 Bearer token의 JWT UID로 본인 범위를 결정한다.
- 작품·사진·받은 좋아요 집계와 최신순·조회순·좋아요순·댓글순을 제공한다. 최대 20개씩 추가로 읽고 UID
  중복을 제거하며, 페이지 또는 새로고침 실패 시 이미 본 작품을 유지하고 명시적 재시도를 제공한다.
  썸네일은 서버의 `/upload/thumbnails/` 경로만 허용한다.
- 본인의 공개·비공개 작품을 함께 표시하고 비공개 상태를 잠금 배지와 VoiceOver 값으로 알린다. 비공개
  작품은 공개 상세 서비스로 우회하지 않고 기존 인증 세션으로 `/board/view`와 댓글 목록을 읽어 본인만
  열 수 있다. 사진 업로드 진입과 업로드 성공 뒤 새로고침도 스튜디오에 연결했다.
- 전체 단위 테스트 111개와 스튜디오·계정·피드·좋아요·업로드·큰 글자 UI 회귀 테스트 5개가 통과했다.
  Release simulator build와 Debug 정적 분석을 통과했고, 정렬·페이지 추가 로드·비공개 상세 화면을
  캡처로 확인했다. 자동 서명 Debug 앱을 연결된 iPhone 17에 설치했다. GOAPI·Android 계약과 운영 서버
  배포 변경은 필요 없다.

## 내 작품 목록 시각 보정 (2026-09-06)

- 작품 미리보기를 기존 4:3 혼합 배치에서 모든 화면 크기의 1:1 정사각형으로 통일했다. 첫 보정은 외부
  `frame`보다 먼저 이미지를 잘라 테스트 placeholder에서는 정사각형처럼 보였지만, 실제 비동기 사진은
  포트레이트가 위아래로, 가로 사진이 좌우로 카드 경계를 넘었다. 제품 소유자의 실기기 캡처로 이를
  확인했다.
- 최종 보정은 일반 글자의 96pt 또는 접근성 큰 글자의 가용 폭을 썸네일 내부의 명시적 정사각형
  `frame`으로 먼저 확정한 다음 `scaledToFill` 이미지를 `clipped()`한다. 따라서 원본 비율과 관계없이
  중앙 1:1 crop만 남고, 그 컨테이너에 12pt continuous rounding과 얇은 윤곽을 적용한다.
- 시스템 `List`가 여러 행 사이에 짧고 비대칭으로 표시하던 기본 구분선을 제거했다. 각 작품을 16pt
  continuous rounding의 독립 카드로 나누고 좌우·상하 여백을 통일해 행 경계를 선 없이 구분한다.
- 최종 보정 뒤 전체 단위 테스트 111개와 일반·접근성 큰 글자 스튜디오 UI 테스트 2개, Release
  simulator build와 Debug 정적 분석을 통과했다. 자동 서명 Debug 앱을 iPhone 17에 다시 설치했으며,
  제품 소유자가 운영 사진의 최종 1:1 crop과 카드 경계가 실기기에서도 정상임을 확인했다.

## 새 업적 축하와 확인 (2026-09-06)

- Android·GOAPI와 같은 인증 사용자 전용 `GET /auth/user/achievements`와
  `PATCH /auth/user/achievements` 계약을 연결했다. 로그인·세션 복원·앱 활성화·새로고침, 댓글 작성과
  사진 업로드 성공 뒤 미확인 업적을 다시 읽고 최대 10개를 한 번에 한 장씩 축하한다.
- 확인이 서버에 저장된 뒤에만 다음 업적으로 넘어간다. 저장 실패 시 현재 업적과 재시도를 유지하고,
  사용자가 명시적으로 `나중에`를 선택했을 때만 현재 실행에서 숨긴다. 로그아웃·계정 전환·만료 응답은
  대기열과 진행 중 응답을 초기화해 다른 계정의 업적이 표시되지 않게 한다.
- 축하 화면은 일반·접근성 큰 글자에서 스크롤 가능한 단일 카드로 제공하며 reduce motion·transparency를
  존중한다. `내 업적 보기`는 기존 공개 사진가 화면의 업적 진열장으로 이동하므로 별도 프로필 계약이나
  서버 변경은 없다.
- 신규 단위 테스트 4개와 업적 대기열·본인 프로필 이동·접근성 큰 글자 UI 테스트 2개가 통과했다. 영향
...
## 1:1 메시지 읽음·polling·사진 탐색 (2026-09-06)

- 운영 반영된 GOAPI `f6250c1` 계약에 맞춰 대화 이력의 `readAt`을 해석하고, 화면에 표시한 마지막 수신
  메시지 UID까지 `PATCH /chat/read`로 읽음 처리한다. 현재 사용자가 보낸 마지막 메시지 한 건에만
  `전송됨` 또는 `읽음`을 표시해 각 말풍선에 상태가 반복되지 않게 했다.
- 대화 화면이 foreground에 있을 때만 12초 간격으로 이력을 갱신하고, 화면 이탈·background에서는 task를
  취소하거나 조회를 건너뛴다. polling 실패는 기존 대화와 초안을 지우지 않으며 새 수신 메시지의 읽음
  요청이 실패하면 다음 polling에서 다시 시도한다. WebSocket·일일 이메일·사진 첨부는 추가하지 않았다.
- 대화 제목에 상대 프로필 사진을 표시하고 사진이 없거나 로드에 실패하면 기본 인물 아이콘을 사용한다.
  메시지 본문의 Unicode 문자·숫자·밑줄 해시태그를 link로 만들고, 선택하면 기존 탐색 화면을 해시태그
  모드와 해당 검색어로 바로 연다. `C#`처럼 단어 내부의 `#`은 link로 만들지 않는다.
- 읽음 request/response·인증·DTO·해시태그 link 단위 테스트 5개와 메시지 목록·알림·사진가 진입 UI
  테스트 3개를 통과했다. 해시태그 link가 `#여름사진` 탐색으로 이동하고 상대 avatar·읽음 상태·전송을
  같은 UI 흐름에서 확인했다. Release simulator build와 Debug 정적 분석도 통과했다. 전체 단위 120개 중
  기능 관련 5개와 나머지 114개는 통과했고 기존 Keychain simulator 환경 테스트 1개는 전체 실행에서만
  실패했으나 단독 실행은 통과했다.

## 1:1 메시지 화면 플랫폼 정합성 보정 (2026-09-06)

- iOS 고유의 native navigation·TextField·keyboard 동작은 유지하면서 Android와 같은 NUBO 웜톤
  라이트·다크 배경, surface, 수신·발신 말풍선 색을 대화 화면에 적용했다. 앱 공통 accent도 같은
  테라코타 계열로 지정해 버튼과 link의 브랜드 연결을 시작했다.
- 대화 제목뿐 아니라 모든 수신·발신 말풍선 옆에 각각 상대와 내 프로필 이미지를 표시한다. 이미지가
  없거나 로드에 실패하면 기존 `AccountAvatar`의 기본 프로필 아이콘을 그대로 사용한다.
- 마지막 메시지 뒤에 24pt 하단 앵커를 두고 화면 재진입, 새 메시지 수신·전송, 키보드 표시 때 그
  앵커로 이동한다. 따라서 입력 영역 바로 위가 답답하지 않고, 다른 화면을 다녀와도 최신 대화가 보인다.
- 전송 시각과 마지막 발신 메시지의 `전송됨`·`읽음`을 같은 메타데이터 행으로 정리했다. 개인정보 경고,
  2,000자 계수와 위쪽 화살표 전송 버튼은 기존 iOS 구현을 유지해 Android와 동일한 정보 구조가 됐다.
- 메시지 계약 단위 테스트 5개와 목록·알림·사진가에서 대화로 진입하는 UI 테스트 3개, 자동 서명 iPhone
  Debug 빌드를 통과했다. 개선본을 iPhone 17에 설치했고 기기 잠금으로 자동 실행만 생략했다.

## 1:1 메시지 프로필 정렬 보정 (2026-09-06)

- 메시지 말풍선과 프로필 이미지의 세로 정렬을 하단 기준에서 중앙 기준으로 변경했다. 여러 줄 메시지나
...
- 제품 소유자가 실제 iPhone에서 댓글·좋아요 사진 활동 push 수신과 알림 선택 시 해당 사진 상세가 열리는
  흐름을 확인했다. 사진 활동 push와 deep link QA는 완료로 본다.

## 사진 업로드 태그 칩과 유사 태그 추천 (2026-09-06)

- 제품 소유자가 운영 사진 업로드 성공을 확인했다. JPEG·HEIC 다중 선택 조합의 별도 QA와 Android 수준의
  자르기·회전·필터 편집은 후속 범위로 남아 있다.
- 쉼표 문자열 한 칸이던 태그 입력을 별도 칩 목록으로 바꿨다. 콤마·스페이스·Return 또는 추가 버튼으로
  태그를 확정하며, `#`로 시작한 입력도 정리한다. 칩 전체를 누르면 삭제할 수 있고 중복, 2~30자 길이와
  한글·영문·숫자·밑줄·마침표 규칙을 입력 위치에서 안내한다.
- 기존 인증 `GET /editor/suggestion/tag`를 200ms debounce로 연결했다. 2자 이상 유효한 입력에 유사 태그와
  사용 횟수를 보여주고 선택하면 곧바로 칩이 된다. 서버·Android·NUBO 계약 변경이나 운영 배포는 없다.
- 태그 파싱·검증·추천 계약 단위 테스트 3개를 추가해 전체 단위 127개가 통과했다. 로그인부터 업로드
  화면까지 이어지는 UI 회귀에서 콤마·스페이스 칩 생성, 추천 선택과 칩 삭제를 확인했고 캡처도 검토했다.
  Release simulator build, Debug 정적 분석과 서명 iPhone 17 Debug build·설치·실행을 통과했다.
- 제품 소유자가 실제 iPhone에서 태그 칩의 생성·삭제와 Android에 가까운 사용성을 확인했다. 이 범위의
  제품 QA는 완료로 본다.

## 사진 업로드 회전·반전·필터 편집 (2026-09-06)

- 선택한 사진을 눌러 개별 편집 화면을 연다. Android와 같은 시계 방향 90도 회전, 좌우 반전, 초기화와
  원본·선명·따뜻함·차가움·필름·흑백 여섯 필터를 제공하고 필터 강도를 조절한다. 편집 여부는 업로드
  썸네일에도 표시한다.
- 편집은 업로드 준비가 끝난 원본 JPEG를 보존하는 비파괴 상태로 관리한다. 미리보기는 축소본에 즉시
  반영하고 완료할 때만 최종 JPEG를 한 번 렌더링한다. 다시 편집하거나 사진을 제거하거나 화면을 닫을 때
  이전 산출물과 원본 임시 파일을 구분해 정리한다.
- 최종 JPEG는 기존 EXIF 촬영 정보를 유지하되 GPS와 이전 orientation을 제거하고 실제 픽셀 방향을
  orientation 1로 고정한다. 회전 뒤 가로·세로 치수, 반전·흑백 결과, EXIF 보존과 GPS 제거를 단위
  테스트로 검증했다. GOAPI·Android·NUBO 계약과 운영 배포 변경은 없다.
- 전체 단위 테스트 129개, 회전·반전·필터·강도·완료와 기존 태그 흐름을 잇는 UI 회귀 테스트가 통과했다.
  Release simulator build와 Debug 정적 분석을 통과했고 자동 서명 Debug 앱을 Wi-Fi로 연결된 iPhone 17에
  설치·실행했다. 자유 비율 자르기는 제스처와 좌표 변환을 별도로 검증해야 하므로 다음 수직 작업으로 둔다.
- 제품 소유자가 실제 iPhone에서 여섯 필터와 강도, 회전·반전·초기화의 동작과 사용성을 확인했다. 이
  범위의 제품 QA는 완료로 본다.

## 사진 업로드 자르기 (2026-09-06)

- 사진 편집에 자르기를 추가했다. 원본 전체가 보이는 검은 작업대 위에 바깥 영역을 어둡게 가린 3×3
  프레임을 표시하고, 프레임 안쪽을 끌어 이동하거나 네 모서리의 큰 손잡이로 크기를 바꾼다. Android의
  원본·4:5·3:4 프리셋과 iOS용 자유 비율을 제공한다.
- 자르기 화면은 최대 1600px 미리보기를 비동기로 준비하되 선택 영역은 원본 기준 정규화 좌표로 보존한다.
  최종 렌더링은 원본 자르기, 회전·반전, 필터 순서로 한 번만 수행하므로 조작 순서와 미리보기 크기에
  관계없이 원본 픽셀에 같은 영역이 적용된다. 기존 EXIF 보존·GPS 제거와 4096px 제한도 유지한다.
- 취소는 편집 상태를 바꾸지 않고 적용한 자르기는 사진별로 독립 보존한다. 다시 편집하면 이전 렌더 파일만
  제거하고 원본은 유지하며, 사진 제거·업로드 화면 종료 시 원본과 산출물을 함께 정리한다.
- 자르기 프리셋·이동·모서리 크기 조절의 경계, 실제 상단 왼쪽 원본 픽셀, 회전 뒤 출력 치수, 사진별 상태와
  임시 파일 교체·정리를 단위 테스트로 검증했다. 전체 단위 테스트 132개와 자르기 취소·적용부터 회전·
  반전·필터·태그까지 잇는 UI 회귀가 통과했다. Release simulator build와 Debug 정적 분석을 통과했고
  자동 서명 Debug 앱을 Wi-Fi iPhone 17에 설치·실행했다. 서버·Android·NUBO 변경은 없다.
...
- Android·NUBO 웹과 같은 인증 `GET/POST /auth/user/report`, `PUT/DELETE /auth/user/block` 계약을
  재사용했다. 사용자와 사진 신고 사유는 공백을 정리한 UTF-16 5~500자로 제한하고 사진 신고에는 게시글
  UID를 포함한다. 신고 성공 상태는 사진 상세과 사진가 프로필, 1:1 대화에서 함께 유지한다.
- 다른 사진가 프로필에서 신고와 차단·해제를 제공한다. 차단하면 작품·통계·업적과 대화 진입을 숨기고,
  1:1 대화에서도 같은 메뉴를 제공해 차단 즉시 이력과 초안·입력창을 제거한다. 사진 상세는 다른 사용자의
  사진을 신고할 수 있고 이미 차단한 사용자의 사진 본문을 표시하지 않는다.
- 로그인 피드와 검색은 Android처럼 인증 요청으로 전환해 서버 응답의 사용자별 `blackList`를 적용한다.
  현재 화면에서도 세션의 차단 상태를 즉시 필터링하며, 로그아웃·계정 전환·토큰 만료 때 신고·차단 캐시를
  초기화한다. 로그아웃 피드의 익명 계약은 그대로다.
- 신고·차단 endpoint·Android 호환 JSON·실패 후 재조회·세션 초기화·인증 피드 필터 단위 테스트 6개를
  추가했다. 전체 단위 138개와 사진 신고→프로필 차단·해제→대화 차단 자동 UI 흐름, Release simulator
  build와 Debug 정적 분석을 통과했다. 서버·Android·NUBO runtime 변경이나 운영 배포는 없다. 제품
  소유자의 요청에 따라 실기기 확인은 남은 작업 완료 뒤 한 번의 통합 QA에서 진행한다.

## 본인 게시글 수정·삭제 (2026-09-06)

- 사진 상세의 작성자 UID가 현재 로그인 사용자와 같고 게시판·카테고리 계약이 유효할 때만 `내 사진 관리`
  메뉴를 표시한다. 수정은 Android와 같은 `PATCH /editor/modify` multipart 계약을 사용해 제목·설명·태그만
  바꾸고 기존 첨부 사진과 공개·비공개 상태를 유지한다.
- 수정 시 업로드 화면과 같은 콤마·스페이스·Return 태그 칩, 칩 삭제, 인증 태그 추천을 제공한다. 제목은
  2~299자, 설명은 2자 이상이며 태그는 업로드와 같은 2~30자 문자 규칙을 적용한다. 성공하면 상세와 피드·
  스튜디오를 다시 불러와 변경 내용을 맞춘다.
- 삭제는 확인 뒤 기존 `DELETE /board/remove/post` JSON 계약을 사용한다. 성공 시 상세를 닫고 피드와 내
  작품 목록에서 즉시 제거하며 작품·사진·조회·좋아요·댓글 요약도 먼저 보정한 다음 서버 값으로 재동기화한다.
- multipart 필드·비공개 보존·기존 사진 무첨부, 인증 수정·삭제, 태그 칩·추천과 즉시 목록 보정을 단위 테스트
...
- Android와 NUBO 웹이 공유하는 라이트 배경 `#F8F3EB`, 다크 배경 `#1C1612`, 라이트 primary
  `#B2583A`, 다크 primary `#DB8F6C`와 surface·container·outline·error 계층을 동적 SwiftUI 팔레트로
  만들었다. 선택 버튼과 발신 말풍선은 각 모드의 `onPrimary`를 사용해 다크 모드의 밝은 테라코타 위에서도
  글자 대비를 유지한다.
- 계정·가입·비밀번호 재설정·탐색·사진가·상세 메타데이터·알림·1:1 메시지·내 작품·업로드·편집·자르기·
  댓글 수정·게시글 수정·신고·계정 삭제 화면에 같은 배경과 surface 계층을 적용했다. 사진 자체가 중심인
  전체 화면 피드·확대 감상·자르기 작업대의 검은 배경은 의도적으로 유지한다.
- 라이트·다크·접근성 큰 글자 캡처에서 계정 시트, 상세 메타데이터, 내 작품, 태그 칩, 편집·자르기 화면의
  배치와 대비를 확인했다. 팔레트의 모드별 실제 RGB를 검증하는 단위 테스트를 추가해 단위 테스트 147개가
  모두 통과했다.
- UI 테스트 32개 중 30개를 한 묶음으로 통과하고, Xcode 27 beta의 한글 키보드 전체선택과 접근성 요소
  분류·스크롤 변동에 영향을 받은 댓글 교체와 큰 글자 업적 2개는 자동화 입력·대기 조건을 보강한 뒤 각각
...
- `SENSTA 1.0 (1)`을 App Store Connect에 실제 업로드했다. Apple의 package 분석과 SPI 분석을 통과해
  처리 완료와 제출 전 검증을 통과했다. 대한민국 단독 무료 배포, 수동 출시, 15+ 국내 연령 등급,
  사진 및 비디오·소셜 네트워킹 카테고리, 6.9형 스크린샷 5장과 버전 메타데이터를 저장했다. 운영
  심사 계정과 연락처를 등록하고 App Privacy 13개 데이터 유형을 게시한 뒤 App Review에 제출했으며,
  현재 상태는 `심사 대기 중`이다. 다음 작업은 Apple 심사 메시지에 대응하고 승인 후 출시 시점을 정해
  수동으로 공개하는 것이다.
