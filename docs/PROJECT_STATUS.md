...
- 이메일 로그인·Keychain 세션 복원·토큰 갱신·로그아웃, 게시글·댓글 좋아요와 댓글 작성·답글을
  구현했다. 다음은 본인 댓글 수정·삭제다.
...
1. 제품 소유자가 앱 종료·재실행 후 세션 유지를 확인했다. 피드·내 계정의 프로필 사진과
   낮은 높이·블러 로그인 시트의 실기기 디자인을 확인한다.
2. 게시글 상세 좋아요·취소와 피드 숫자 반영을 실기기에서 확인한다. 다음은 댓글 작성·답글과
   댓글 좋아요를 기존 인증 요청·토큰 갱신에 연결하는 작업이다.
3. Google 로그인용 iOS OAuth client와 Apple 로그인을 준비한다. 설정이 필요하면 제품 소유자에게
   브라우저 작업을 구체적으로 안내한다. 기존 Android 호환성을 유지한다.
4. GOAPI 변경이 필요하면 로컬 Colima를 시작하고 `./scripts/build-ubuntu22.sh`로 교체용 파일을 만든다.
