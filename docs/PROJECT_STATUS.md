...
1. 실제 iPhone에서 알림 미확인·읽음 처리와 JPEG·HEIC 다중 업로드 조합을 마무리 검증한다.
2. 업로드 사진 자유 비율 자르기를 iOS 방식으로 구현하고 회전·반전과 조합한 좌표, 적용·취소와 다중
   사진별 상태를 검증한다.
3. 신고와 차단을 작은 수직 기능 단위로 구현한다. 서버 계약 변경이 필요하면 웹·Android·iOS 영향을 함께
   확인하고 제품 소유자에게 먼저 알린다.
4. 본인 게시글 수정·삭제를 작품 스튜디오와 상세에 연결한다.
5. 앱 내 계정 삭제와 Sign in with Apple token 폐기, UGC 심사 시나리오와 App Store 제출 준비를 진행한다.
6. 웹 Universal Link 범위는 원격 push 출시를 막지 않으므로 별도 작업으로 둔다.
